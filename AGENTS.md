...
4. **加载顺序即依赖顺序**。`index.html` 里 `<script>` 的先后顺序定义了模块依赖关系，
   **不得调整顺序**，也不得改用 `type="module"`（会破坏 `file://` 可用性）。
   顺序：`core → audio → fx → ui → items → weapons → enemies → boss → gen → build → player → game → main`
5. **注释使用简体中文**。
...
8. **禁止把「计划」写成「已完成」**。`docs/PROJECT_STATUS.md` 里三态必须分清：
   已完成 / 部分完成 / 计划中。
9. **不要删除 `bt_*.html`**。它们是历次自测通过的完整页面快照，是本项目没有 git 的情况下
   唯一的历史凭证。
...
- 读 `<title>`，形如 `BOOTTEST_PASS_P44_F0` 即 44 通过 0 失败
