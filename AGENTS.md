...
| 代码量 | `js/` 下 16 个文件，约 7900 行 |
| 自测 | `index.html?boottest`，48 步，结果写进 `document.title` 与页面底部 `#errlog` |
| 当前自测状态 | **48 PASS / 0 FAIL**（2026-09-02 实测，含 BUG-001 修复、主角重做回归、拍立得/武器商店/商店通行/赌徒的灾难回归步骤） |
...
4. **加载顺序即依赖顺序**。`index.html` 里 `<script>` 的先后顺序定义了模块依赖关系，
   **不得调整顺序**，也不得改用 `type="module"`（会破坏 `file://` 可用性）。
   顺序：`core → audio → fx → ui → items → weapons → shop → photo → gambler → enemies → boss → gen → build → player → game → main`
5. **注释使用简体中文**。
...
- 读 `<title>`，形如 `BOOTTEST_PASS_P48_F0` 即 48 通过 0 失败
