...
| 自测 | `index.html?boottest`，61 步，结果写进 `document.title` 与页面底部 `#errlog` |
| 当前自测状态 | **61 PASS / 0 FAIL**（2026-09-04 实测，含 BUG-001 修复、主角重做回归、拍立得/武器商店/赌徒的灾难/解锁与词缀/第三层与无面君主/第三层新怪/基地系统/武器批次（纸飞机/吹风机/点唱机/悖论骰子重做）/音频系统 2.0/三合一改动（吹风机增强+删除切割刀·太阳左轮+点唱机网络重构）/删除敌人批次（Wallmaker·猎犬下架）/基地视觉重制 2.0（多区域 Hub+中央核心+NPC 造型/动画+UI 可读性分层）/基地扩展批次（地图 32×20+加宽过道+HTML 世界标签清晰化+面板不遮对话）/基地反馈批次（武器架任选·深渊祝福·弹药补给·训练靶反馈·标签遮挡·展示亭·战利品墙·非矩形外框）/基地微调批次（拆隔断墙·删展示亭·HUD武器显示·训练靶分散）/基地核心护栏拆除·外墙恢复·标题菜单重做/局外成长轨道A·B（装甲舱·重力靴·深渊共鸣）批次回归步骤） |
