...
| 代码量 | `js/` 下 23 个文件，约 12147 行 |
| 自测 | `index.html?boottest`，62 步，结果写进 `document.title` 与页面底部 `#errlog` |
| 当前自测状态 | **62 PASS / 0 FAIL**（2026-09-04 实测，含 BUG-001 修复、主角重做回归、拍立得/武器商店/赌徒的灾难/解锁与词缀/第三层与无面君主/第三层新怪/基地系统/武器批次（纸飞机/吹风机/切割刀/太阳左轮重做/点唱机/悖论骰子重做）/Wallmaker·猎犬/音频系统 2.0 回归步骤） |
...
4. **加载顺序即依赖顺序**。`index.html` 里 `<script>` 的先后顺序定义了模块依赖关系，
   **不得调整顺序**，也不得改用 `type="module"`（会破坏 `file://` 可用性）。
   顺序：`core → audio → fx → ui → items → weapons → jukebox → scalpel → sunrevolver → dice → shop → photo → gambler → meta → base → enemies → boss → voidking → gen → build → player → game → main` |
5. **注释使用简体中文**。
...
├── js\                       ← 23 个 JS 文件（22 个 IIFE 模块 + main.js 测试套件，见 §1.4 的加载顺序）
