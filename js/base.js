...
const B = {
  ...
  makeFloor(){
    ...
    const stubs={ '7,0':1,'7,1':1, '14,0':1,'14,1':1 };   // 北侧两个隔断短墙（南侧敞开通行）
  ...
  _props(room){
    // 深渊升降梯（唯一出本入口）
    {
      ...
      g.add(M(pgeo('lift', b=>{
        b.box(0,.14,0,1.9,.28,1.9,0x3c4048);
        b.box(0,.3,0,1.5,.1,1.5,0x50565e);
        b.box(-.85,1.0,-.85,.16,1.7,.16,0x2c3036); b.box(.85,1.0,-.85,.16,1.7,.16,0x2c3036);
        b.box(-.85,1.0,.85,.16,1.7,.16,0x2c3036); b.box(.85,1.0,.85,.16,1.7,.16,0x2c3036);
        b.cyl(0,2.2,-.4,.05,.05,2.6,0x584428,5); b.cyl(0,2.2,.4,.05,.05,2.6,0x584428,5);
      }),0,0,0));
      const ring=new THREE.Mesh(new THREE.RingGeometry(.5,.72,18), G.bmat(0x8a5aff,.5));
      ring.rotation.x=-Math.PI/2; ring.position.y=.31; g.add(ring);
      this.addProp(room,{type:'gate',x:2.5,z:7.5,r:.8,hp:Infinity,blocksMove:true,blocksBullets:true,mesh:g,
        interact:{label:()=>'乘升降梯 · 下潜至第一层 [深渊碎片 '+G.meta.data.shards+' ◆]', range:1.7,
          fn:()=>{ G.game.launchRun(); }}});
      this.tag('深渊升降梯','#c8a9ff',2.5,2.7,7.5,1.5);
    }
    // 枪械师工作台 + 武器架交互点
    {
      const g=new THREE.Group();
      g.add(M(pgeo('bench', b=>{
        ...
        b.box(-.7,.25,.3,.12,.5,.12,0x54402a); b.box(.7,.25,.3,.12,.5,.12,0x54402a);
        b.box(.2,.62,0,.3,.08,.2,0x3a3f4a);
        b.box(-.3,.6,.1,.24,.06,.16,0x8a6a3a);
      }),0,0,0));
    ...
    // 武器架交互点（循环试用已解锁武器）
    {
      ...
      g.add(M(pgeo('rackStand', b=>{
        b.box(0,.55,0,.24,1.1,.24,0x54402a);
        b.box(0,1.0,0,.7,.08,.3,0x6a4c2e);
        b.box(0,.14,0,.5,.1,.5,0x443424);
      }),0,0,0));
      this.addProp(room,{type:'rackUse',x:2.6,z:5.6,r:.45,hp:Infinity,blocksMove:true,blocksBullets:false,mesh:g,
        interact:{label:()=>{
          const cur=G.player && G.player.weapons[G.player.curW];
          return '试用武器'+(cur?'（当前：'+cur.def.name+'）':'');
        }, range:1.5, fn:()=>{ this.cycleWeapon(); }}});
      this.tag('武器架 · [E] 试用','#ffe9a0',2.6,1.75,5.6,1.3);
    }
    // 医疗舱（免费治疗 + 等级展示）
    {
      const g=new THREE.Group();
      g.add(M(pgeo('medbay', b=>{
        ...
        const cross=new THREE.Group();            // 绿十字用自发光材质：320p 俯视角下可辨识
      ...
      this.tag('医疗站 Lv'+G.meta.up('medbay'),'#7ae8b0',8.5,1.9,2.3,1.2);
    }
    // 弹药工作台 / 工程机械 / 地图桌 / 书架 / 木箱（氛围+阻挡）
    {
      ...
      this.tag('弹药工作台','#d8cdb4',19,1.5,2.4,1.1);
      ...
      this.tag('深渊测绘桌','#d8cdb4',14.6,1.35,11.2,1.1);
      mk('shelf1', b=>{
      ...
      for(const [cx,cz] of [[12.2,12.6],[13.0,12.8],[12.6,13.4]])
        mk('crate'+cx, b=>{
    ...
    // 训练靶（可射击，打碎自动重置）
    {
      ...
      const hp=60+lv*120;
      for(const [dx,dz] of [[6.2,10.9],[7.7,11.7],[9.2,10.9]]){
        const g=new THREE.Group();
      ...
      this.tag('训 练 场','#ffe9a0',7.7,2.5,12.6,1.6);
    }
  },
  ...
  _npcMesh(key){
    ...
    if(key==='gunsmith'){
      add('nGS_body', b=>{
        b.box(0,.55,0,.56,.6,.4,0x6a5a3a);
        b.box(0,.5,.21,.5,.5,.06,0x7a5230);            // 皮围裙
        b.box(0,.98,0,.4,.34,.34,0x8a7454);
        b.box(-.1,1.12,.18,.09,.06,.03,0x5ad0d8); b.box(.1,1.12,.18,.09,.06,.03,0x5ad0d8);  // 额前护目镜
        b.box(-.24,.75,0,.14,.4,.14,0x6a5a3a);         // 左臂
      });
      const arm=new THREE.Group();
      arm.add(M(pgeo('nGS_arm', b=>{
        b.box(0,-.2,0,.14,.44,.14,0x6a5a3a);
        b.box(0,-.46,.05,.18,.1,.12,0xd8cba8);         // 擦枪布
      }),0,0,0));
      arm.position.set(.26,.85,0); g.add(arm); refs.arm=arm;
      refs.workFace=0;                                  // 面向工作台（西）
    } else if(key==='engineer'){
      add('nEG_body', b=>{
        b.box(0,.52,0,.5,.56,.38,0x5a6a5a);
        b.box(-.2,.85,-.2,.3,.44,.2,0x4a4438);         // 工具背箱
        b.box(-.2,1.1,-.2,.34,.08,.24,0x3a342c);
        b.cyl(.3,.95,.1,.03,.03,.5,0x8a8a92,5);        // 背包螺丝刀
        b.box(0,1.06,0,.38,.3,.34,0x9a8a6a);
        b.cyl(0,1.28,0,.24,.24,.1,0xe0c040,8);         // 安全帽
        b.cyl(0,1.24,0,.3,.3,.03,0xe0c040,8);
      });
      const arm=new THREE.Group();
      arm.add(M(pgeo('nEG_arm', b=>{
        b.box(0,-.2,0,.13,.42,.13,0x5a6a5a);
        b.box(0,-.48,0,.06,.06,.34,0xb0b4bc);          // 扳手
      }),0,0,0));
      arm.position.set(.27,.82,0); g.add(arm); refs.arm=arm;
      refs.workFace=Math.PI;                          // 面向西侧的工程机械
    } else if(key==='archivist'){
      add('nAR_body', b=>{
        b.cone(0,.6,0,.36,.9,0x4a5a48);
        b.sph(0,1.12,0,.2,0xc8b494);
        b.box(-.07,1.14,.16,.06,.04,.02,0x2a2a30); b.box(.07,1.14,.16,.06,.04,.02,0x2a2a30); // 眼镜
      });
      const arm=new THREE.Group();
      arm.add(M(pgeo('nAR_book', b=>{
        b.box(0,-.3,.1,.34,.42,.1,0x6a3a2e);           // 大书
        b.box(.02,-.3,.16,.3,.38,.02,0xe8dcb8);        // 书页
      }),0,0,0));
      arm.position.set(.1,.95,.1); g.add(arm); refs.arm=arm;
      refs.workFace=0;                                // 面向东侧书架
    } else { // instructor
      add('nIN_body', b=>{
        b.box(0,.55,0,.5,.62,.36,0x5a6a48);
        b.box(0,.52,.19,.42,.5,.05,0x4a5a3a);
        b.box(0,1.0,0,.36,.32,.32,0xc8a882);
        b.cyl(0,1.26,0,.24,.24,.1,0x4a5a3a,8);         // 军帽
        b.cyl(0,1.2,0,.28,.28,.03,0x4a5a3a,8);
        b.box(0,1.1,.19,.05,.05,.05,0xe8c15a);         // 哨子
        b.box(-.25,.78,0,.13,.44,.13,0x5a6a48);
      });
      const arm=new THREE.Group();
      arm.add(M(pgeo('nIN_arm', b=>{
        b.box(0,-.2,0,.13,.42,.13,0x5a6a48);
        b.box(0,-.44,0,.09,.09,.2,0xc8a882);           // 指向前方的手
      }),0,0,0));
      arm.position.set(.27,.85,0); arm.rotation.x=-.5; g.add(arm); refs.arm=arm;
  ...
  _armory(room){
    ...
    const show=unlocked.slice(0,5);
    show.forEach((id,i)=>{
      const def=W.defs[id];
      const g=G.build.props.wrack(def, TIER_COLOR[def.tier]);
      g.position.set(1.5, 0, 6.6+i*1.55);
      g.rotation.y=Math.PI/2;   // 枪口朝向室内（东）
      const halo=new THREE.Sprite(G.pmat(TIER_COLOR[def.tier]));  // 常亮辉光（B.update 的呼吸辉光较暗，320p 出图下增强可辨识度）
      halo.scale.set(1.3,1.3,1); halo.position.y=.86; g.add(halo);
      G.world.add(g);
      room.wrackGroups.push(g);
      this.tag(def.name, TIER_COLOR[def.tier], 2.2, 1.5, 6.6+i*1.55, 1.0);
    });
    this.tag('武 器 展 示 架','#d8cdb4',2.6,2.4,5.2,1.3);
  },
  ...
  _lamps(room){
    ...
    for(const [x,z] of [[4,.8],[11,.8],[18,.8],[.8,7],[20.2,7],[5.5,13.6],[15.5,13.6]]){
      const g=new THREE.Group();
      g.add(M(pgeo('lamp', b=>{
        b.cyl(0,.35,0,.05,.07,.7,0x3a342c,5);          // 吊杆
        b.cyl(0,.62,0,.16,.1,.2,0x50453c,6);           // 灯罩
      }),0,0,0));
      if(!_lampMat) _lampMat=new THREE.SpriteMaterial({map:G.tex('flame'),transparent:true,depthWrite:false});
      const fl=new THREE.Sprite(_lampMat); fl.scale.set(.6,.66,1); fl.position.y=.5; g.add(fl);
      g.userData.flame=fl;
  ...
  update(dt){
    ...
    // 炉火余烬 + 空气尘埃（低频，复用 fx 粒子池）
    this._emberT-=dt;
    if(this._emberT<=0){
      this._emberT=.22;
      G.fx.particle(13.2,.7,1.9,{vx:(Math.random()-.5)*.4,vy:1.1,vz:(Math.random()-.5)*.4,life:.5,color:0xffb050,s0:.1,kind:'a'});
    }
    this._dustT-=dt;
    if(this._dustT<=0){
      this._dustT=.5;
      G.fx.particle(p?p.x+(Math.random()-.5)*8:13, .3+Math.random()*1.4, p?p.z+(Math.random()-.5)*8:9,
