...
W.init = function(scene){
  ...
  for(let i=0;i<MAXB;i++){
    ...
    this.bullets.push({ on:false, mesh, glow, x:0,z:0, vx:0,vz:0, ang:0, spd:0, dmg:0, size:.1,
      team:'p', pierce:0, bounce:0, knock:0, life:0, crit:false, kind:'', hits:null, dmgDecay:1, color:0xffffff, wid:'', sunP:false });
  }
};
...
W.spawn = function(o){
  for(let i=0;i<MAXB;i++){
    ...
    if(!b.on){
      ...
      b.sunP=o.sunP||false;   // 太阳之弹：PERFECT 标记（更大弹体 / 更强爆发 / 更亮演出）
      const m=b.mesh;
      ...
      if(b.kind==='rocket'||b.kind==='plasma'||b.kind==='bomb'||b.kind==='voidorb'||b.kind==='sun'){
        b.glow.material = G.pmat(b.kind==='sun'?0xffd070:b.color,'a'); b.glow.visible=true;
        const gs = b.kind==='sun'?(b.sunP?1.5:1.2):(b.kind==='rocket'?1.1:(b.kind==='bomb'?1.0:.8)); b.glow.scale.set(gs,gs,1);
...
W.spawnPlayer = function(p, ang, def, wid, mul){
  ...
  const dmgMul = p.curDmgMul() * (mul||1);   // mul：献给太阳的左轮的 Heat 伤害倍率（sunrevolver.mulOf）
...
W.update = function(dt){
  ...
  for(let i=0;i<MAXB;i++){
    ...
    if(b.life<=0){
      ...
      if(b.kind==='sun' && G.sunrevolver) G.sunrevolver.sunBurst(b.x,b.z,b.sunP);   // 太阳之弹寿命终结：整房爆发
    ...
    for(let s=0;s<n && !dead;s++){
      ...
      if(solid){
        ...
        else if(b.kind==='sun' && G.sunrevolver){ G.sunrevolver.sunBurst(b.x,b.z,b.sunP); }
        else impactFx(nx,nz,b.color);
        dead=true; break;
      }
      b.x=nx; b.z=nz;
      ...
      for(const pr of G.props){
        ...
        if(dx*dx+dz*dz < (pr.r+b.size)*(pr.r+b.size)){
          ...
          else if(b.kind==='sun' && G.sunrevolver){ G.sunrevolver.sunBurst(b.x,b.z,b.sunP); dead=true; }
      ...
      if(b.team==='p'){
        for(const e of G.enemies.list){
          ...
          if(dx*dx+dz*dz < rr*rr){
            ...
            // 太阳之弹：命中即蒸发演出（设计稿十五，不是传统爆炸）；pierce 99 继续贯穿
            if(b.kind==='sun' && G.sunrevolver) G.sunrevolver.sunHit(b, e, e.x, e.z);
        ...
        if(!dead && e && !e.dead && e.spawnT<=0){
          ...
          const dx=e.x-b.x, dz=e.z-b.z, rr=e.r+b.size;
          if(dx*dx+dz*dz < rr*rr){
            // 太阳之弹对 Boss 单次封顶（BOSS_CAP）：与切割刀/点唱机同一纪律，避免传奇武器秒杀 Boss
            if(b.kind==='sun' && G.sunrevolver) G.hurtBoss(Math.min(b.dmg, G.sunrevolver.K.BOSS_CAP));
            else G.hurtBoss(b.dmg * (e.stunT>0?1.5:1));
    ...
    // 太阳之弹的三层视觉 / 灼热轨迹 / 环境照明由 sunrevolver.update 统一驱动，此处不再重复发射粒子
    else if(b.kind==='sun'){ /* 见 js/sunrevolver.js */ }
