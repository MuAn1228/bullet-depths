...
async function runBootTest(){
  ...
  await step('63_基地反馈批次：武器架任选·深渊祝福·弹药补给·训练靶·标签遮挡', async ()=>{
    ...
    // ①d 边界墙恢复：南北外墙 z=0 / z=19 应为墙，东西墙保留
    assert(get(16,0) && get(16,0).t==='wall','北外墙 z=0 未恢复');
    assert(get(16,19) && get(16,19).t==='wall','南外墙 z=19 未恢复');
    assert(get(0,10) && get(0,10).t==='wall','东西外墙应保留');
    // ①e 深渊核心左右护栏已拆（x=11 / x=21 短墙应为地板）
    assert(get(11,9) && get(11,9).t!=='wall','核心左侧护栏未拆除');
    assert(get(21,9) && get(21,9).t!=='wall','核心右侧护栏未拆除');
    // ①f 玩家向北推到底仍被外墙挡住（不越界）
    G.player.x=16; G.player.z=1.2;
    G.input.key['KeyW']=true; frames(60); G.input.key['KeyW']=false;
    assert(G.player.z>0.3 && G.player.z<2.0,'边界墙缺失或移动异常 z='+G.player.z);
