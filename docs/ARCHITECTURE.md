...
├── js\weapons.js  (~480 行)  20 种武器定义 / 品阶统一定价 / 子弹对象池 / 弹道 / 命中 / 爆炸 / 电弧链 / 黑胶互撞检测（wid 击杀归属 + 纸飞机/吹风机/点唱机/太阳弹种接线，sun 弹命中/爆发分发）
...
├── js\main.js    (~2160 行) 启动引导 + 59 步自测套件（自测占约 1990 行）+ 截图模式（shop/map/2/base）
└── lib\three.min.js
```
