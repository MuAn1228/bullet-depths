...
├── js\base.js     (935 行)  基地「废弃军械站」：32×20 多区域 Hub（武器工坊/核心大厅/工程区/训练场/休息区/仓库展厅/档案角，双 tile 门洞宽过道）+ 中央深渊核心「破晓引擎」+ NPC×4（独立造型/Idle 动画/看向玩家）+ 数据驱动对话（大对话框，门店看板内嵌引言不遮挡）+ 门店面板 + 分区彩色灯光 + 环境事件（核心环转/蒸汽/灯闪/炉火）+ Meta 成长可视化（武器架/靶场/档案角）+ HTML 高分辨率世界标签层（CSS px 悬浮文字）
