...
├── js\weapons.js  (372 行)  15 种武器定义 / 品阶统一定价 / 子弹对象池 / 弹道 / 命中 / 爆炸 / 电弧链（wid 击杀归属）
├── js\shop.js     (279 行)  武器商店：目录 UI / 与当前武器对比 / 购买事务（验金→扣款→给予，防重复）
...
├── js\enemies.js  (1034 行)  15 种敌人定义 / 造型 / AI / 生成 / 受伤 / 死亡 / 自愈 / 照片状态进出 / 虚空护壁
...
├── js\main.js    (1947 行)  启动引导 + 53 步自测套件（自测占约 1700 行）+ 截图模式（shop/map/2/base）
└── lib\three.min.js
```
