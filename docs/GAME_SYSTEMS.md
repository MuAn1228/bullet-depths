...
### 4.1 定义表 `E.defs`（`enemies.js:182-199`）—— 共 **30 种**

> 2026-09-04 删除批次：Wallmaker 掩体制造者（§4.9）与 Hound 猎犬（§4.10）因设计不佳整体下架，
> 计数 17 → 15 → 22 → 30（2026-09-04 新增 6 种机制型 + 拟态怪 mimic + 8 种机制型批次：miner/vaultling/barrier_brute/footballer/jester/podcaster/magnetron/balloon_wisp）；后续小节编号 4.9/4.10 不再重排（下架记录见 `DEVELOPMENT_LOG`）。
...
| slime       | 13 | 3.2  | .51 | 1    | \[1,2]   | `:535`  | 弹跳推进（2026-09-06 机动+体积）；**死亡分裂**成 2 只小史莱姆（r=.36）                                                                                                                                                    |
...
| totem       | 40 | 0    | .42 | 2    | \[1,2]   | `:676`  | 静止，双臂激光 1.05 rad/s 旋转扫射 3.2s（2026-09-06 判定对齐+攻击性提升）                                                                                                                                              |
| bomber      | 34 | 1.9  | .38 | 2    | \[2]     | `:719`  | 保持 4.5\~6.5，抛射炸弹（落地爆炸半径 1.9 / 伤害 2）                                                                                                                                       |
| voidstalker | 24 | 3.6  | .34 | 2    | \[3]     | `:925`  | **虚空掠影**：半透明蛇形潜行 → 闪现玩家背后 1.7 格 → 显形预警 0.5s → 突刺（9.5 速 0.24s，命中后自设 contactCd 防双扣）→ 硬直 0.7s                                                                                |
...
| orbiter      | 20 | .9   | .36 | 2    | \[1,2]   | `:30722+` | **环形放射者**：悬浮核心，蓄力 1.15s 连续 8 波 360° 环形弹（奇偶环错位留可穿缝隙）                                                                                                    |
| minelayer    | 30 | 1.8  | .4  | 1    | \[1,2]   | `:30722+` | **地雷工兵**：巡逻布设「滚动地雷」（慢速炸弹+红圈预警，碰触引爆半径 1.9 / 伤害 2）                                                                                                            |
| gravitator   | 36 | 1.2  | .42 | 2    | \[2]     | `:30722+` | **引力眼球**：远距漂浮，周期 1.6s 持续将玩家吸向自己（引力波，玩家可对抗）                                                                                                                |
| commander    | 44 | 1.3  | .44 | 2    | \[2,3]   | `:30722+` | **战场指挥官**：指挥光环加速 6 格内同袍攻速（atkCd 额外推进 .5×）+ 自身扇形 5 发齐射 ×3 波                                                                                                 |
| mirror       | 38 | 1.6  | .4  | 2    | \[2]     | `:30722+` | **镜面反射者**：持镜盾，正面格挡并折射一颗高速反击弹；5 次命中破防踉跄 2.5s（详见 §4.11）                                                                                                    |
| phaseprowler | 26 | 2.4  | .36 | 2    | \[3]     | `:30722+` | **相位潜行者**：隐形蛇形逼近 → 显形三连斩（每刀 1 伤）→ 隐身撤退（详见 §4.12）                                                                                                            |                                                                                                          |
| mimic       | 24 | 2.5  | .4  | 2    | \[2,3]   | `:AI mimic`| **拟态怪**：伪装成宝箱完全静止；靠近 1.2 格/按 E 互动/受击 → 揭示 → 张嘴扑击（接触 2 伤）→ 5~7 枚短程扇形弹 → 正常追逐（详见 §4.13）
| miner       | 24 | 2.5  | .3  | 2    | \[2,3]   | `:AI miner`  | **挖掘者**：CHASE→DIG（钻地预警 0.6s）→UNDERGROUND（1.5~2.2s，免疫普弹，地面土痕可追踪）→EMERGE（土堆预警 0.5s）→ATTACK 扑击 4.5~5.5 速不锁定→RECOVER。逼玩家回头，与 Sniper 协同 |
| vaultling   | 21 | 2.6  | .3  | 2    | \[2,3]   | `:AI vaultling`| **跳跃者**：PREPARE 下蹲 0.5s+跳跃轨迹预警→VAULT 长跳 3.5~5 格（空中可受远程伤害、不穿外墙）→LAND 轻微冲击波 1 伤→RECOVER 0.4s。可跳 Wallmaker/小型敌人防线 |
| barrier_brute| 42 | 1.75 | .42 | 3    | \[2,3]   | `:AI barrier_brute`| **路障蛮兵**：正面实体护甲（独立耐久 22），正面普弹减 70% 消耗护甲，背后/爆炸/ignoreBlock 全额；护甲归零→BERSERK 狂暴（移速×1.3、接触伤+1）。与 Shield 累计格挡破防形成不同优先级 |
| footballer  | 47 | 2.0  | .42 | 3    | \[3]     | `:AI footballer`| **橄榄球狂徒**：低速推进→CHARGE_PREPARE 0.7s 冲锋路线预警→冲锋 6~7 速 0.8~1.2s（受击×0.5、撞开小型敌人、撞玩家 2 伤）→撞墙眩晕 1.1~1.4s 输出窗口 |
| jester      | 23 | 2.2  | .32 | 2    | \[2,3]   | `:AI jester`  | **小丑**：Bullet Twist 干扰场（r4.5，CD5~7s）：普通实体弹进场偏转 15~35°（一次性），激光/爆炸/近战/电弧/特殊 IgnoreBlock 免疫（weapons.js `b.aj` 标志+def 声明豁免） |
| podcaster   | 25 | 1.6  | .34 | 3    | \[3]     | `:AI podcaster`| **阵型指挥者**：Rally 施法 1.0~1.3s 发光集结→真实移动重排周围敌人（前排盾卫/蛮兵/冲锋，中排散弹/机枪，后排狙击/治疗），空间不足放弃排序；CD 8~12s，不瞬移不改属性 |
| magnetron   | 30 | 1.65 | .36 | 2    | \[3]     | `:AI magnetron`| **磁铁怪**：Magnetic Field（r3.5，2~3s/CD4~6s）渐进转向吸附普通金属弹并储能（≤10，亮度随储能），0.8s 蓄力释放储能×1 枚环形弹（weapons.js `b.am` 标志）；与 Jester 偏转明确区分 |
| balloon_wisp| 16 | 2.25 | .3  | 2    | \[3]     | `:AI balloon_wisp`| **气球怨灵**：悬浮空中慢飘（距玩家 5~8 格），每 3.5~5s 锁定玩家地面位置 0.8s 蓄力+地面预警圈→投 Void Bomb（爆炸 r1.5、2 伤）；可被 Shotgun/爆炸/穿透空击，不可穿房间边界 |                                      |                                                                                                          |
...
- `AI` 是文件底部的对象字面量（`enemies.js:640-1029`），22 个键与 `defs` 一一对应
...
slime 且 gen===0 → 分裂 2 只（hp=7, r=.36），显式继承 e.room
...
| ---- | ----------------------- | ------------------------------ | ----------------------------------------------------------- |
| 战斗房数 | 7                       | 9                              | 10                                                          |
| 敌人池  | 6 种基础                   | 12 种混合                         | 9 种高阶（sniper/hexer/bomber 加权，无 gunner/charger/slime/shroom） |
...
**第四层「失序维度」专属生成器（`gen4.js`，2026-09-05）**：

与前三层 `gen.js` 的随机房间拼图完全不同，第四层使用独立生成器：

| 项 | 第四层「失序维度」 |
|---|---|
| 生成器 | `gen4.js`（独立，不复用 gen.js） |
| 布局算法 | 中央核心 core（2×2 椭圆平台）+ 6~7 主方向放射 + 二/三圈延伸 + 回环连接（节点图，非规则网格） |
| 地图边界 | 17×15 cells（前三层 13×11，约 1.8 倍面积） |
| 连接模型 | 桥房（bridge 房间，狭长地板通道）+ 标准 4-tile 门；BFS 最短路铺桥，可汇接既有桥形成枢纽 |
| 房间 shape | core / ring / platform / fracture / corridor / tiered / bridge / rect 八种掩码（bbox 仍矩形，掩码决定非矩形地板） |
| 虚空 | 不写 tile（碰撞天然 solid、子弹被吞——刻意取舍） |
| 房间数 | 25~45（300 种子统计 avg 32.2） |
| 敌人池 | 远程弹幕↑ + 地形控制 + 冲锋的空间压力组合，按 shape 加权，精英率 55% |
| Boss | 无面君主强化为「终焉回响」（HP 1600、体型 1.12、专属血条与横幅） |
| 终点 | Boss 击杀即通关（无 exit 房） |

**四种地图机制**（`gen4.js` 布置 + `build.js` 渲染 + `G.gen4.update` 驱动）：

| 机制 | 行为 |
|---|---|
| 相位桥 phaseDoors | 周期开闭，锁定战斗/实体站门上时不关 |
| 空间折叠门 foldGates | 成对双向传送 + 去向标牌，可理解（2 对，选相距最远区域） |
| 裂缝锚点 riftAnchors | 交互撕开隐藏通路（秘密入口的可读开启方式） |
| 引力井 wells | 周期拉拽 + 旋涡粒子（Boss 竞技场必配 + 随机 1 个大战斗区） |

**生成器鲁棒性**：全图 BFS 连通校验 + 10 次种子重试 + 2 次独立种子兜底；
`layBridge` 原子化回滚（失败回滚已建桥房与门）；`hookToLinked` 兜底（arm 连不上核心时挂到当前已连通房间）。
300 种子大样本失败率 0%（2026-09-05 修复 layBridge 汇接缺陷后）。
...
**三层领主攻击强化（2026-09-06，用户反馈「Boss 太弱/血条异常」）**：
`weapons.js` 敌弹命中玩家原为**固定 1 hp**（`b.dmg` 在玩家判定处是摆设）——新增 `hdmg` 重击弹透传，
只有 Boss 弹显式携带才按 2 hp（一整心）结算，普通敌人弹/爆炸路径零改动。无面君主：长枪 3→4 连发 2hp、
接触 2hp、花瓣 P1 3 臂/P3 5 臂、环 16/20、瞬移 10 向、墙弹 4.2。空间裂解者：空间弹 2hp、接触实装 2hp、
裂解区实装（1.2s 预警闪烁→圈内 2hp 爆裂→余时减速领域）、裂缝滞留 1.1s/1hp 放电、扇形 10 发/螺旋 14 发、
节奏全面缩短。失序之主：追踪弹 2hp（P3 4 连发）、接触实装 2hp、扇形 9/11、螺旋 14、冷却 1.7/1.15、
瞬移 4.5/3.0s、召唤 3 只/8s。FIX-032：voidripper 阶段切换曾把 2/3 字面量当血量比例传 bossBar（血条 200%/300%），
已改传真实比例。三 Boss maxhp（1150/1200/1600）被 STEP 45/72/74 断言锁死，强化只动攻击端。

**第五层黑化重制（2026-09-06，用户反馈「绿色不如黑色/怪物数量不够」）**：
...
> **2026-09-04 视觉重制 2.0**：地图升级为多区域 Hub（z=5/z=10 隔断分区——北区武器工坊/
> 核心大厅/工程区，南区训练场/休息区/仓库展厅，中区走廊连档案角与地牢入口）；中央视觉
> 焦点「破晓引擎」深渊核心（旋转符文环+呼吸水晶+紫光+四角符文柱）+ 地牢入口第二焦点
> （机械门楣+符文灯+深红紫辉光+地面符文环）；四名 NPC 独立造型（工作服围裙护目镜弹匣 /
> 工装背包机械手套电线安全帽 / 长外套眼镜地图包 / 护甲护腕军靴训练棍）+ Idle 动画与
> 看向玩家；分区彩色挂灯 + 6 个受控 PointLight + 环境事件（核心环转/蒸汽/灯闪/炉火）；
> 基地环境音增强（机械运转+火炉噼啪）；Meta 成长可视化（武器架/靶场/档案角随升级增多）。
> 功能区域坐标与地图结构详见 `base.js` `makeFloor`/`_props`/`_lamps`/`_npcMesh`。
> **2026-09-04 微调**：南北隔断墙已拆除、**深渊核心左右护栏（x=11/x=21）已拆除**（核心
> 四周全开放）、南北外墙（z=0/z=19）恢复为墙、武器展示亭已删除（武器架挑选面板保留）、
> 基地 HUD 右下显示当前武器、训练靶分散大三角。标题菜单已重做为像素风深渊地牢背景
> （中央紫光核心/旋转符文/浮尘粒子/金色艺术字，z-index 提至 41 避开 #fade 黑场）。
...
- **训练靶**=dummy prop（blocksBullets+hp，靶盘直径 1.1）：`B.damageProp` 专用分支——
  伤害数字 + 金属火花 + 靶盘微颤 + `clank` 音效；打碎后 1.1s 自动重置、永不死亡不掉落
  不计击杀；训练场 HTML「命中 N 次」实时计数标签。
- **武器架**\[E] 打开商店式**全武器任选面板**（`renderPanel('weapons')`）：列出所有已解锁
  武器（品阶排序），点「试用」直接 `p.weapons[curW]=mktWeapon(id)` 装备到手（不收费、
  不生成掉落、任意换、当前武器金框 `.wcard.cur`）。
- **弹药工作台**（ammoBench prop）：\[E] 免费补满当前武器弹匣（`w.ammo=w.def.mag`）+ 音效/粒子。
- **深渊核心「破晓引擎」**（core prop）：能量柱 + 双地面符文圈 + 顶喷粒子 + 献祭交互——
  \[E] 消耗 8 碎片 → `G.meta.data.bless` 累计；`startRun()` 进本每层祝福伤害 +15%
  （`p.st.dmgMul*=1.15^b`）后清零。
- **战利品墙**=壁龛陈列柜（大背板带框+展台柱盘+铭牌台+背光），Boss 首杀点亮、未首杀「？？？」。
- **基地微调**（2026-09-04）：南北隔断墙（z=6/z=13）已拆除、武器展示亭已删除；基地 HUD
  显示当前武器（品阶色名+弹药）；训练靶分散成大三角（lv0 两座、lv1 三座）。
- **基地外框**：四角切角 + 墙端内收 + 东西墙齿状凹凸（非矩形，同地牢房间感）。
- **标签遮挡**：`openPanel()` 打开任意面板时隐藏 `#tagLayer`，关闭恢复，外部标签不叠数据板。
- 展示架/战利品/图签随 meta 实时成长（进基地重建 + 面板内购买即 `rebuildScene()`）。
HUD：`#baseHud`（♥ / ◆ / 操作提示）替换战斗 HUD。
