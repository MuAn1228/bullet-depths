...
| wallmaker   | 26 | 1.5  | .36 | 2    | \[2,3]   | `:1218` | **掩体制造者**：保持 4~~7 距巡逻 → 找墙位（玩家↔后排连线中点）→ 0.9s 蓄力（地面蓝环预警+举锤）→ 生成临时魔法墙（≤3 面、寿命 6s、拆最老）→ 技能 CD 5~~8s；五道软锁防线见 §4.9                                                             |
| hound       | 19 | 3.2  | .30 | 1    | \[1,2,3] | `:1305` | **猎犬**：高速近战压迫——CHASE(≤2.8 解锁扑击) → PREDICT(玩家翻滚时用当前运动状态有限预测落点，连续同向翻滚预测更准) → 0.45s 预警(停步低伏+地面方向线+吼叫) → LEAP(6.5 速定向前扑 0.45s，不穿墙) → RECOVER(扑空/撞墙 0.5\~0.7s 后摇=输出窗口)，见 §4.10 |
...
### 4.9 Wallmaker 掩体制造者与临时魔法墙（`enemies.js`，2026-09-04 新增）

战场控制型敌人（第 2/3 层，`floors:[2,3]`）：hp 26 / spd 1.5 / r .36 / cost 2 /
money 2-4。**不造成直接伤害**，职责=生成临时墙切断玩家与后排敌人的射线、改变走位。

**AI 状态机**（映射设计稿）：`idle`（PATROL/REPOSITION：保持 4~~7 距 + 横向游走，技能
CD 5~~8s）→ `position`（找墙位：优先「玩家↔最近 2 个敌人」连线中点偏玩家侧，其次玩家
四周环形采样；无合法位则回巡逻短重试）→ `cast`（0.9s 蓄力：停止移动 + 地面蓝环每帧
预警 + 举锤发粒子，玩家看得见"这里要出墙"）→ 落地前**再跑一次 wallLegal 兜底** → 回
idle。满 6s 后墙自然消失。蓄力中可被击杀打断。

**魔法墙（运行时 prop）**：type `'wall'`，r .55、hp 80、life 6s、上限 **3**（超出拆最老）。

- 挡**敌方**子弹（blocksBullets）——Sniper/Gunner 弹可被玩家借来当掩体（设计稿七：
  "墙必须能帮玩家"）；**玩家子弹穿透**（weapons.js 特判，同翻桌逻辑——不卡自己输出）。

- 挡玩家移动（blocksMove），但五道软锁防线保证永不把玩家堵死（见下）。

- 可被爆炸/范围伤破坏（`B.damageProp` case 'wall'，hp 80）；`updateWalls` 每帧自愈清理
  已被打碎的墙引用；**换房即拆**（墙绑定生成时的房间，玩家换房后自动移除，防跨房残留）。

- 计数/清理：`E.walls` 数组；`E.clear`（新局/换层）摘 mesh；damageProp 的 splice 由
  updateWalls 跟随清理。

**五道软锁防线（`E.wallLegal`，缺一不可）**：
① tile 必须为 floor（不生成进墙/门/空隙）② 不与任何 blocksMove 道具或已有墙重叠
③ 离玩家中心 ≥1.05（不生成在玩家脚下）④ 距任一 door tile ≤1 cell 拒绝（不堵出口）
⑤ **BFS 可达性**：玩家所在 cell 必须仍能到达至少一个打开的门（无打开的门=战斗锁定中
不构成软锁，放行）。⚠️ 门 tile 位于两房交界、bbox 之外一格，BFS 遍历范围须扩出
bbox ±1——否则门永远不可达（2026-09-04 实测踩坑）。

回归锁：自测 STEP 60（选点/上限 3 拆最老/玩家弹穿透/敌弹被挡/寿命/爆炸破坏/软锁拒绝/
真实 AI cast→生成链路）。

### 4.10 Hound 猎犬与翻滚落点有限预测（`enemies.js`，2026-09-04 新增）

高速近战压迫型（全楼层 `floors:[1,2,3]`）：hp 19 / spd 3.2 / r .3 / cost 1 / money 1-3。
**不发射弹幕**，靠「预测玩家翻滚落点」的扑击施加压力——目的是**防止无脑连续翻滚**，
而非禁翻滚（设计稿一/九）。

**AI 状态机**：

- `chase`：追击 1.5\~2.5 窗口（>2.5 逼近、<1.5 稍退，不持续贴脸）；距玩家 ≤2.8 且冷却
  就绪 → 普通扑击；**玩家滚动中（p.rollT>0）且 ≤4.5 → PREDICT**（调用 `E.rollPredict`）。

- `windup`：0.45s 预警——停止移动、低伏、地面红色方向线（预告扑击路径，临扑转亮）、
  `houndGrowl` 吼叫；玩家有充足反应时间。

- `leap`：6.5 u/s 定向前扑 0.45s（约 2.9 格），方向扑前锁定、**扑中不转向**；
  moveEntity 碰撞保证**不穿墙**；命中玩家走通用接触伤害（翻滚中免疫）。

- `recover`：扑空/撞墙后摇 0.5~~0.7s（撞墙更长），玩家主要输出窗口；结束回 chase 且
  技能冷却 0.8~~1.6s，不会无限连续扑。

**翻滚落点有限预测（`E.rollPredict(h,p)`，红线级纪律）**：

- 只用玩家**当前运动状态**推算终点：`剩余翻滚距离 = p.rollT × 14`（翻滚固定 14 u/s）、
  方向 `rollAng` 恒定——**绝不读取未来坐标**（设计稿十三）。

- **反翻滚博弈**：连续同向翻滚（|ΔrollAng|<0.35）→ streak 递增（封顶 3）→ streak≥2 时
  预测点取**精确落点**（更难躲）；方向骤变 → streak 归 1 并给预测点附加侧向随机偏移
  （±0.8\~1.7，扑空率高）。玩家「翻滚后立刻变向」即可破解——博弈成立。

回归锁：自测 STEP 61（翻滚预测单测（streak×3 精确点/变向重置）/真实 AI 链路
（翻滚→windup→leap→recover→回追击+冷却）/预警方向线存在与清理）。
