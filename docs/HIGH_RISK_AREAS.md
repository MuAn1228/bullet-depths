...
## H27. 猎犬的翻滚落点预测：有限预测的纪律（2026-09-04 新增）

**背景**：`E.rollPredict(h,p)`（enemies.js）用于 `hound` AI 在玩家翻滚时推算落点以扑击。

**红线**：
1. **只用当前运动状态**：`剩余翻滚距离 = p.rollT × 14`、方向 `rollAng` 恒定——这是
   「读玩家当前状态 + 匀速外推」，不是读未来坐标。若有人改成「读 p 翻滚后的真实落点」
   （如直接采样几帧后的坐标），本质是作弊 AI，玩家会感觉"这游戏读我操作"（设计稿十三
   的明确禁止项）。
2. **反翻滚博弈必须保留**：连续同向翻滚 streak 递增 → 预测精确；变向 → streak 归 1 且
   预测带侧向随机偏移（扑空率高）。删掉偏移/取消 streak 依赖会回到「100% 命中翻滚」
   的硬克制，摧毁"改变方向即可破解"的博弈。
3. 扑击**不穿墙**：LEAP 用 `moveEntity`（碰撞天然挡墙），撞墙进更长后摇。若改成直接
   传输/无视墙 → Hound 能跨墙咬人，与设计稿十一冲突。
