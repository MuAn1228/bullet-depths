...
- **bridge 房的门必须贯通到 5-tile 中带**（2026-09-06 实测教训）：门开在中带外时 doorGuarantee
  只向内铺 2 格，门后是 2 格地板+墙 = 宽死路（实测 5% 死桥）。bridge case 铺完中带后必须对每个门 tile
  补 1-tile 宽连接带到中带（立足格用 `G.clamp` 到房间边界，门 tile 可能落在 x0-2/x1+2）。探针：
  `C:\Users\liboh\AppData\Local\Temp\probe_gen4_300.js`（300 种子 0 失败基线）。
