...
## 2026-09-05（第三层卡顿修复：过载点唱机 vinyl 粒子系统优化）

- **背景**：用户报告在第三层同时持有「赌徒的灾难」和「过载点唱机」时，打着打着突然出现严重卡顿，无法继续游戏。
- **根因分析**：
  ① **vinyl 拖尾粒子爆炸**（主因）：weapons.js 第 491-497 行，每个 vinyl 子弹每帧 80% 概率生成 2 个粒子（红蓝双色拖尾），寿命 0.22s。vinyl 上限 16 发 → 每秒生成约 1536 个粒子 → 同时存在约 **338 个**，接近粒子池上限 340（**99% 满负荷**）。加上敌人受击粒子、环境特效、JOKER/FULL OVERLOAD 爆发粒子，一旦触发固定时间步长死亡螺旋（game.js 第 920 行最多补 4 帧），帧率骤降导致严重卡顿。
  ② **撞墙 ring 无冷却**：weapons.js 第 388 行，vinyl 子弹每次撞墙都生成 ring 特效 + 音效，配合 bounce:99 在第三层复杂地形中频繁反弹，短时间内大量生成 ring。
  ③ **排除磁铁怪交互**：weapons.js 第 180 行确认过载点唱机（def.jukebox）标记为 isSpecial，第 196 行 `!isSpecial` 豁免磁铁怪磁场，vinyl 子弹不受磁铁影响，排除此路径。
- **修复内容**（weapons.js）：
  ① **拖尾粒子优化**（第 491-499 行）：
     - 生成概率：80% → **30%**
     - 粒子数量：2 个（红蓝同时）→ **1 个交替红蓝**
     - 粒子寿命：0.22s → **0.12s**
     - 效果：同时存在粒子从 ~338 降到 **~35**（**减少 90%**），红蓝双色色差观感保留
  ② **撞墙 ring 冷却**（第 388 行）：添加 **0.12s 冷却**（b._bounceCd 字段），防密集反弹时 ring/音效刷屏，拖尾粒子块中同步递减冷却计时
- **验证**：
  - node --check 语法通过（weapons.js）
  - boottest 连跑 3 轮：**BOOTTEST_PASS_P69_F0**（69 PASS / 0 FAIL），无 flake
  - 理论性能提升：粒子系统负荷从 99% 降到 ~10%，预留充足余量给战斗特效
- **版本号**：weapons.js v=32 → **v=33**
