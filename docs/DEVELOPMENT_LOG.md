...
## 2026-09-06（冰晶散射者重做：A 价位数据对齐 + 冻结 3 秒；STEP 冻结断言适配）

- **用户反馈**：冰晶散射者太垃圾，卖得却很贵（A 档 50 弹壳，与火箭筒/磁轨/雷暴同价）。
- **弱因**：单轮爆发 5×2.3=11.5，**打不过 32 价的 C 档双管粉碎者（6×2.8=16.8）**；贯穿 0、弹匣 6、
  射程 7；唯一卖点减速幅度 45% 被 STEP 断言锁死不可动。
- **重做**（身份仍是「散射+冰冻」）：6 弹丸 ×3.2（爆发 19.2）、**贯穿 1 敌**、弹匣 6→8、射速 1.8→2.0、
  装填 1.6→1.5、散布 .22→.2、弹速 12→13、射程 7→8、击退 3→4、**冻结时长 2→3s**（weapons.js `b.slow`
  唯一来源即 frost，时长未被断言锁）。价格保持 50——数据对齐后物有所值。
- **STEP 适配**：冻结断言的枪手测试靶（16hp）会被新爆发秒杀——死亡敌人不再执行减速结算
  （enemies.js:914 只对活敌跑），靶子加血到 100 保证测的是减速而非击杀（main.js v86→87）。
- **验证**：boottest ×3 全绿（改后首跑 2 轮 FAIL 即上述秒杀问题，靶子加血后全绿）；
  实机数值实测（手动步进，面板失焦 RAF 暂停时 sleep 不会推进游戏——boottest 同款教训）：
  单弹伤害 3.2（靶伤全为 3.2 倍数）、贯穿第二靶掉血 ✓、slowT≈2.6（3s 减飞行）✓、弹匣 8→7 ✓。
- **版本**：weapons 39 / main 87，index.html 已 bump。

---

## 2026-09-06（第五层密度二次加密：战斗节点 22%→34% + 全特殊房敌波上调 + 银行守卫）

- **用户反馈**：第五层的怪太少，没有打击感（黑化重制批次的密度上调后仍不够）。
- **floor5.js**：
  - 节点特殊房概率 `.74→.66`——战斗房节点占比 22%→34%（生成后 `ratio>=.55` 校验仍兜底，实测 8 种子特殊房占比全过）；
  - 战斗房预算 `5+面积*2.4→6+面积*3.0`、单体扣减 1.5→1.4、上限 16→20——2×2 房约 12 只、3×2 房约 15 只（双波）；
  - 精英率 `[.08,.12,.22,.32]→[.10,.18,.28,.38]`。
- **特殊房敌波二次加密**：weaponchaos 7→10（**仍单波**，STEP 74 约束）、darkness 9→12、collapse 9→12、
  altar 6→9、theft 8→11、giant 3~4→4~5 只、swap 3→5 只、vote 三档 4/4·5/6·7/5/7；
  **弹壳银行加 5 只守卫**（原版完全无战斗像走过场；完成条件不变——购买或 10s 兜底，守卫不阻断流程，
  clearRoom 只开门不清怪、残敌随玩家离开留在房内）；宝箱风暴敌箱权重 1/7→2/8、每箱出 2 只。
- **验证**：boottest ×6（5 绿 + 1 次历史 STEP 11 隐藏房 flake，`隐藏门未生成`，KNOWN_ISSUES 在案，与本批次文件无关）；
  实机：3×2 战斗房双波 [11,6] 共 17 只含 2 精英、生成侧 8 种子平均每层约 46 只敌人波（原约 25）、UPDATE-FAIL=0。
- **版本**：floor5 6 / rooms5 7 / rooms5b 5，index.html 已 bump。
...
- **🔴 血条 bug 根因（FIX-032）**：`voidripper.js` 阶段切换时 `G.ui.bossBar(true,名, 2/3)`——第三参是 0~1 的**血量比例**，
  传字面量 2/3 即血条瞬间涨到 200%/300%，下次 `hurt()` 才被真实比例打回（=「异常增高再缩减」）。
  修复：传 `boss.hp/boss.maxhp`。voidking/anomaly 复查无同类问题（每帧传真实比例）。
- **🔑 弱的根因（系统性）**：`weapons.js` 敌方子弹命中玩家**固定扣 1 hp**（`p.hurt(1,...)`）——
  Boss 弹上的 `b.dmg`（12/10/8）在玩家判定处从来只是摆设。玩家 6 hp + 护甲整击吸收 + 0.9s 无敌帧
  ⇒ Boss 全套弹幕是「半颗心刮痧」。另外 voidripper 的战场裂解区**纯视觉零交互**、悬浮核心**无接触伤害**。
- **weapons.js（v37→38）hdmg 重击弹透传**：弹池加 `hdmg` 字段，命中玩家按 `b.hdmg||1` 结算。
  **只有显式携带 hdmg 的 Boss 弹才按 2 hp（一整心）结算**——普通敌人子弹、爆炸路径（dmg>10?2:1）零改动。
- **voidking.js（v2→3）无面君主**：虚空长枪 3→4 连发（spd 7.2→7.8）且 **2 hp 重击**；花瓣螺旋 P1 2→3 臂 / P3 4→5 臂、
  转速 .1/.085→.095/.08；同心环 14/18→16/20 发；瞬移 8→10 向弹；弹幕墙 3.8→4.2；**接触伤害 1→2 hp**。
- **voidripper.js（v6→7）空间裂解者**：空间弹 **2 hp 重击**（spd 5→5.4）；扇形 8→10 发；螺旋 12→14 发；
  P1/P2/P3 攻击冷却 2.2/1.8+1.5/1.2 → 1.8/1.5+1.3/1.0；召唤上限 4→5、冷却 10→9；
  **接触伤害实装**（原版贴身白嫖，2 hp/0.9s）；**战场裂解区实装**（原版纯视觉）：1.2s 预警闪烁 → 落点爆裂
  （圈内 2 hp + 击退 + 爆闪）→ 余时减速领域，冷却 7→5.5s、上限 2→3；**空间裂缝滞留放电**（1.1s 一次 1 hp）。
- **anomaly.js（v2→3）失序之主**：三连追踪弹 **2 hp 重击**且 P3 3→4 连发；扇形 7/9→9/11；螺旋 12→14；
  攻击冷却 2.1/1.4 → 1.7/1.15；瞬移 5.5/3.5 → 4.5/3.0s；P3 召唤 2→3 只、冷却 9→8s；**接触伤害实装（2 hp）**。
- **测试约束核对**：三 Boss maxhp 全被断言锁死（1150/1200/1600，STEP 45/72/74）——本批次只动攻击端，HP 零改动；
  STEP 45/72/74 全程 `invulnT=1` 站桩，伤害强化不碰任何断言。
- **验证**：
  - boottest ×3 `BOOTTEST_PASS_P70_F0`（首跑 P69_F1 定位为 anomaly 接触块插在 `const d` 声明前触发 TDZ
    ReferenceError——移到距离计算之后即修复，改后 ×3 全绿）；
  - browser-use 数值级实测：vk 长枪/接触=4（6-2）、vr 空间弹/接触=4、an 追踪弹/接触=4、
    vr 二阶段血条宽度 **60%**（修复前 200%）、UPDATE-FAIL=0；
  - 测试方法学踩坑：① Boss 房正中的 `coredevice` 道具（r2.6 blocksBullets）会吃掉测试弹——
    弹伤实测须把玩家挪到离中心 4 格；② 两次 evaluate 之间页面自行运行，无敌消失的站桩玩家会被 Boss 打死，
    后续断言全部作废——完整重置必须在**单次 evaluate 内**做完。
- **版本**：weapons 38 / voidking 3 / voidripper 7 / anomaly 3，index.html 已 bump。
